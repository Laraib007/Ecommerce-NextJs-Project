   ...
   <h1 className='text-lg text-center font-bold m-2'>This is Store Cart</h1> 
    ...
    <ol className='font-semibold list-decimal'>
      ...
      {Object.keys(cart).map((k)=>{return <li key={k}>
        ...
        <div className='font-bold flex justify-center items-center  w-1/3 '><FaMinusCircle onClick={()=>removeFromCart(k, cart[k].name, cart[k].price, cart[k].qty, cart[k].size, cart[k].varient)} className='text-pink-600 text-sd mx-1' />{cart[k].qty}<  FaPlusCircle className='text-pink-600 text-sd mx-1' onclick={()=>addToCart(k, cart[k].name, cart[k].price, cart[k].qty, cart[k].size, cart[k].varient)} />
        </div>
        </div>
      </li>})}
    </ol>
